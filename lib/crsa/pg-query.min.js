...
function pgFindChangedNodes(a, b, list) {
    if (pgCompareNodes(a, b))return null;
    var cha = [];
    var chb = [];
...
var pgReindentText = function (text, indent) {
    if (!indent)indent = "";
...
pgQuery.prototype.create = function (html) {
    var pgel = pgCreateNodeFromHtml(html);
    this.createFromPgNode(pgel);
    return this
};
pgQuery.prototype.createFromPgNode = function (pgel) {
    var html = pgel.toStringWithIds();
    var $el = $(html);
...
pgQuery.prototype.attr = function (attr, value) {
    for (var i = 0; i < this.list.length; i++) {
        if (typeof value == "undefined") {
            ...
            this.list[i].pgel.setAttr(attr, value, true);
