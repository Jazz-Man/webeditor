...
var getClassesFromSelector = function (sel) {
    ...
    if (m) {
        var temp = {};
        for (var i = 0; i < m.length; i++)temp[m[i]] = true;
        var r = [];
        for (var k in temp) {
            r.push(k)
        }
...
(function ($) {
    ...
    window.CrsaVariable = function () {
        ...
        this.isInUse = function () {
            ...
            for (var i = 0; i < crsaStylesheets.length; i++) {
                ...
                var s = cs.genGetSourceForRules(cs.crsa_rules);
    ...
    window.CrsaStylesheet = function () {
        ...
        this.load = function (url, done, force) {
            ...
            var loadCss = function () {
                ...
                if (isApp() && crsaIsFileUrl(url)) {
                    try {
                        var fileName = crsaMakeFileFromUrl(url, true);
                    ...
                    $.ajax({url: url, data: null, dataType: "text"}).done(function (data) {
                        console.log("loaded css " + url);
            ...
            if (isApp() && crsaIsFileUrl(lessurl)) {
                try {
                    fs = require("fs");
                ...
                $.ajax({url: lessurl, data: null, dataType: "text"}).done(function (data) {
                    console.log("loaded " + lessurl);
                    _this.has_less_file = true;
        ...
        this.save = function (path, done, fs, filter_func) {
            if (!fs)fs = require("fs");
            var cs = this;
                        ...
                        if (_this.crsa_rules[i].media) {
                            ...
                            if (m) {
                                for (var n = 0; n < m.length; n++) {
                                    var b = parseInt(m[n]);
                                    if (_this.crsa_rules[i].media.match(new RegExp("max\\-width\\:\\s*" + b + "px"))) {
                                        b = b + 1
                                    }
                                    if (list.indexOf(b) < 0) {
                                        list.push(b)
        ...
        this.changeBreakpoint = function (old, b, done) {
            var doTask = function () {
                var re = new RegExp(":\\s*" + old + "px", "g");
                var re_small = new RegExp(":\\s*" + (old - 1) + "px", "g");
                var list = [];
                var changed = false;
                if (_this.crsa_rules) {
                    for (var i = 0; i < _this.crsa_rules.length; i++) {
                        if (_this.crsa_rules[i].media) {
                            var nm = _this.crsa_rules[i].media.replace(re, ": " + b + "px");
                            nm = nm.replace(re_small, ": " + (b - 1) + "px");
        ...
        this.processTreeRule = function (rule, crsa_rules, skip_vars) {
            ...
            try {
                if (rule.hasOwnProperty("importedFilename")) {
                    rule.crsaSource = '@import "' + rule.importedFilename + '";'
    ...
    window.CrsaPageStyles = function (iframe) {
        ...
        this.loadAllStylesheets = function (done) {
            var count = 0;
            ...
            for (var i = 0; i < doc.styleSheets.length; i++) {
                ...
                if (sheet.ownerNode) {
                    ...
                    if (!getElementPgId($(sheet.ownerNode))) {
                        continue
                    }
                }
    ...
    var methods = {
        init: function (options) {
            ...
            $("div.canvas").on("crsa-page-closed", function () {
                $iframes = $("iframe.content-iframe")
            });
        ...
        }, loadLessStyles: function (iframe, done) {
            var $iframe = $(iframe);
                    ...
                    var doAdd = function () {
                        ...
                        if (val.length > 0) {
                            ...
                            if (ev) {
                                ...
                                getCrsaPageForIframe($.fn.crsa("getSelectedPage")).undoStack.add("Add variable " + val);
            ...
            var updateList = function () {
                ...
                $dest.html("");
                $("<h2/>").html("Variables").appendTo($dest);
                $.each(crsaStylesheets, function (i, cs) {
                    ...
                    $.each(list, function (j, v) {
                        ...
                        var $fc = $.fn.crsa("addInputField", $c, {
                            type: "stylesheet",
                            data: cs
                        }, v.name, fdef, values, true).data("crsa-var", v);
                        var $del = $("<a/>", {href: "#"}).html('<i class="fa fa-trash-o" />').on("click", function (e) {
                            ...
                            if (v.isInUse()) {
                                ...
                                getCrsaPageForIframe($.fn.crsa("getSelectedPage")).undoStack.add("Remove variable " + v.name);
                        ...
                        if ($input) {
                            $input.on("change input", function (event) {
                                ...
                                if (!undo_recorded || undo_recorded != v) {
                                    getCrsaPageForIframe($.fn.crsa("getSelectedPage")).undoStack.add("Change " + v.name);
            ...
            updateList();
            $("body").on("crsa-rules-changed crsa-stylesheets-changed", function () {
            ...
            var updateList = function () {
                ...
                for (var i = 0; i < list.length; i++) {
                    ...
                    if (selectedPage) {
                        ...
                        if (!cs.isEnabledForPage(selectedPage.get(0))) {
                            $add = $("<a/>", {
                                "class": "crsa-sm-attach",
                                href: "#"
                            }).html('<i class="fa fa-reply"></i> Attach to ' + cp.name).appendTo($li)
                        } else {
                            $li.addClass("crsa-active");
                            $add = $("<a/>", {
                                "class": "crsa-sm-attach",
                                href: "#"
                            }).html('<i class="fa fa-times"></i> Detach from ' + cp.name).appendTo($li)
                        }
                ...
                var tabs = '<ul class="nav nav-tabs">                <li class="active"><a href="#ss-add-new" data-toggle="tab">New...</a></li>                <li><a href="#ss-add-open" data-toggle="tab">Open file...</a></li>                <li><a href="#ss-add-url" data-toggle="tab">Open url...</a></li>            </ul>                <div class="tab-content">                    <div class="tab-pane active" id="ss-add-new"><p class="text-muted">Create a new stylesheet (if you select an existing stylesheet it will be loaded instead).</p></div>                    <div class="tab-pane" id="ss-add-open"><p class="text-muted">Open an existing stylesheet.</p></div>                    <div class="tab-pane" id="ss-add-url"><p class="text-muted">Enter an absolute or relative url of the stylesheet that you want to create or load.</p></div>                </div>';
                if ($tabs)$tabs.remove();
            ...
            var text;
            if (crsaPage) {
                text = "Select which framework components you want to use with this page in <b>Lib</b> and <b>Prop</b> panels. NOTE: This will not add framework stylesheets and scripts to the page."
            } else {
                text = "The list of frameworks, libraries and plugins. Select Framework Manager for a specific page to manage which plugins are active for that page."
            }
            ...
            var showEditForm = function ($li) {
                ...
                $form.find("button.add").on("click", function (e) {
                    ...
                    var file = url;
                    if (pinegrow.findFrameworkWithFile(file)) {
                        $url_group.addClass("has-error");
                        ...
                        if (confirm("Are you sure? Only Pinegrow PLUGINS and COMPONENTS can be loaded. Anything else (framework JavaScript files, etc...) WILL break the app.")) {
                            pinegrow.loadFrameworkFromFile(file, true, function (err) {
                                if (err) {
                ...
                $.each(list, function (i, fm) {
                    ...
                    if (crsaPage) {
                        if (crsaPage.frameworks.indexOf(fm) < 0) {
                            $add = $("<a/>", {
                                "class": "crsa-sm-attach",
                                href: "#"
                            }).html('<i class="fa fa-plus"></i> Activate').appendTo($li)
                        } else {
                            $li.addClass("crsa-active");
                            $add = $("<a/>", {
                                "class": "crsa-sm-attach",
                                href: "#"
                            }).html('<i class="fa fa-minus"></i> Deactivate').appendTo($li)
                        }
                        $add.on("click", function (e) {
                            var $li = $(e.delegateTarget).closest("li");
                            ...
                            if (idx >= 0) {
                                ...
                                if (!crsaPage.addFramework(fm)) {
                                    showAlert("Only one " + fm.type + " framework can be added to a page at the same time. Remove the other one first.", "Could not add framework")
                                } else {
                                    $li.addClass("crsa-active")
                                }
                                updateList();
                    ...
                    if (fm.changed) {
                        $("<a/>", {
                            "class": "crsa-sm-attach",
                            href: "#"
                        }).html('<i class="fa fa-save"></i> <b>Save</b>').appendTo($li).on("click", function (e) {
                            e.preventDefault();
                            var $li = $(e.delegateTarget).closest("li");
                            var fm = $li.data("crsa-fm");
                            if (fm.localFile) {
                                fm.save(fm.localFile, function () {
                    ...
                    if (fm.localFile) {
                        $("<a/>", {
                            "class": "crsa-sm-attach",
                            href: "#"
                        }).html('<i class="fa fa-times"></i> Unload').appendTo($li).on("click", function (e) {
                            e.preventDefault();
                            var $li = $(e.delegateTarget).closest("li");
                            var fm = $li.data("crsa-fm");
                            if (fm.changed) {
                        ...
                        $("<div/>", {}).html(fm.localFile).appendTo($li)
                    }
                });
                var $li = $("<li/>").appendTo($b);
                showEditForm($li)
            ...
            updateList();
            $div.append('<p><a href="#" class="show-load-plugins">Load plugin</a> - <a href="https://github.com/matjaztrontelj/PinegrowCommunity/wiki" target="_blank" class="link">Learn how to create Pinegrow plugins.</a></p>');
            $div.find("a.show-load-plugins").on("click", function (e) {
                e.preventDefault();
                $d.find(".load-plugin").show()
            });
            var $d = makeModalDialog(crsaPage ? "Framework &amp; Plugin Manager for " + crsaPage.name : "Framework &amp; Plugin Manager", "Close", null, $div);
            if (isApp()) {
    ...
    function editRuleSource(rule) {
        var editorData = showCodeEditor("text/css", "Edit CSS rule code", "edit-rule-code", function () {
        ...
        var cs = rule.crsa_stylesheet;
        getCrsaPageForIframe($.fn.crsa("getSelectedPage")).undoStack.add("Edit CSS rule");
    ...
    var filterRules = function ($rules, s) {
        ...
        if (s.split) {
            ...
            s = function () {
                if (!this.selectorText)return false;
    ...
    $.fn.crsacss.defaults = {title: "crsa"};
    ...
    var CrsaClassManager = function ($dest) {
        ...
        var updateStylesheetList = function () {
            ...
            $.each(allSheets, function (i, cs) {
                ...
                var $detach = $("<a/>", {href: "#"}).html("   x");
                var $move = $("<i/>", {"class": "fa fa-bars"}).appendTo($li);
                $fil.tooltip({
                    container: "body",
                    placement: "top",
                    title: "Hide / show stylesheet rules in the list below.",
                    trigger: "hover"
                });
                $vis.tooltip({
                    container: "body",
                    placement: "top",
                    title: "Disable / enable the stylesheet.",
                    trigger: "hover"
                });
                showSSHidden(cs, $li);
        ...
        $("<a/>", {
            "class": "cm-sslist-manage",
            href: "#"
        }).html("+ Manage...").appendTo($rules_div).on("click", function (e) {
            $.fn.crsacss("showStylesheetsManager");
            e.preventDefault()
        });
        $psul.sortable({handle: "i.fa-bars", axis: "y", scroll: false}).on("sortupdate", function (event, ui) {
            var list = [];
        ...
        $("<h2/>").html("CSS Rules").appendTo($rules_div);
        var $controls = $('<div style="position:relative;"></div>').appendTo($rules_div);
        var $input = $("<input/>", {
            "class": "crsa-cm-input form-control filter-form",
            placeholder: "search"
        }).appendTo($controls);
        crsaAddCancelSearch($input, "top: 8px;right: 76px;");
        var $only_active = $('<label class="css-opt-label"><input class="control-label" type="checkbox" value="1">&nbsp;Active</label>').appendTo($controls);
        ...
        $only_active.tooltip({
            container: "body",
            placement: "top",
            title: "Show only active rules for the selected element.",
            trigger: "hover"
        });
        ...
        $only_visible.tooltip({
            container: "body",
            placement: "top",
            title: "Show only rules that match the current window size.",
            trigger: "hover"
        });
        var filter = null;
        ...
        $ul.get(0).addEventListener("click", function (event) {
            ...
            if ($target.closest(".crsa-cm-apply").length) {
                ...
                getCrsaPageForIframe($.fn.crsa("getSelectedPage")).undoStack.add("Remove CSS rule");
                cs.removeLessRule(r);
                cs.genRegenerateAndSetCssSource(function () {
                ...
                if (r.selector && !r.multiple_values) {
                    ...
                    if (r.multiple_values) {
                        crsaQuickMessage("Editing properties is not possible because the rule contains <b>multiple values for the same property</b>.", 4e3)
                    }
                }
                stop = true
            }
        ...
        $ul.get(0).addEventListener("dragstart", function (event) {
            ...
            setTimeout(function () {
                dragged_$el.css("opacity", .4).hide()
            }, 100);
            $.fn.crsa("hidePreview")
        }, true);
        ...
        var updateRulesORIGINAL = function () {
            ...
            $.each(cp.getAllCrsaStylesheets(), function (j, cs) {
                ...
                if (type != "css") {
                    ...
                    $tt.tooltip({
                        container: "body",
                        placement: "auto top",
                        title: "This stylesheet uses " + type.toUpperCase() + " parser.",
                        trigger: "hover"
                    });
                    $li.find("h3").append($tt)
                }
                var $a = $("<a/>", {
                    "class": "cm-sslist-addrule",
                    href: "#"
                }).html("+ Add rule").appendTo($li.find(">div"));
                $a.on("click", function (e) {
                    e.preventDefault();
                    ...
                    $.each(list, function (i, r) {
                        ...
                        if (r.selector) {
                            ...
                            $name = $("<a/>", {
                                "class": "crsa-cm-edit",
                                href: "#"
                            }).text(shortenString(r.raw, 18)).appendTo($div);
            ...
            $ul.find(".crsa-cm-edit").on("click", function (event) {
                var $li = $(event.delegateTarget).closest("li");
                var r = $li.data("rule");
                if (r.selector) {
                    _this.editRule(r)
            ...
            $ul.find("div.rule").on("contextmenu", function (event) {
                event.preventDefault();
                var $li = $(event.delegateTarget).closest("li");
                showContextMenuForRule($li, false)
            });
            $ul.find(".crsa-cm-code").on("click", function (event) {
                var $li = $(event.delegateTarget).closest("li");
                var r = $li.data("rule");
                editRuleSource(r);
                event.preventDefault()
        ...
        this.editRule = function (r) {
            ...
            $main_div.animate({left: -300}, 150, function () {
            })
        };
        this.showListPanel = function (animated) {
            if (animated) {
                $main_div.animate({left: 0}, 150, function () {
                })
            } else {
                $main_div.css("left", 0)
            }
        ...
        var showAddRuleBox = function ($el, cs, value) {
            ...
            var pop = $el.popover({
                ...
                container: "body",
                content: '<form id="' + eid + '"><div class="form-group"><input class="form-control" placeholder=".new-rule" style="margin-bottom:8px;"/><p class="help-block"></p></p><button class="ok btn">Create</button><button class="ok assign btn btn-link">Create & Assign</button><button class="closeit btn btn-link">Cancel</button></div></form>'
            }).on("shown.bs.popover", function () {
                var $d = $("#" + eid);
                var $i = $d.find("input").focus();
                ...
                var doAdd = function (e) {
                    ...
                    getCrsaPageForIframe($.fn.crsa("getSelectedPage")).undoStack.add("Add CSS rule / " + r);
                    cs.addLessRule(r, {});
                    cs.genRegenerateAndSetCssSource(function () {
                        $("body").trigger("crsa-rules-changed");
        ...
        var updateUsedClasses = function () {
            ...
            if (!selectedPage)return;
            var media_vis_cache = {};
            var win = getCrsaPageForIframe(selectedPage).getWindow();
            if ($refElement) {
                var ref_rules_dir = null;
                var node = $refElement.get(0);
                if (node.ownerDocument.defaultView && node.ownerDocument.defaultView.getMatchedCSSRules) {
